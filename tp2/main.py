    ...
    while(j>=0):
        ...
        while i <len(tmpTab):
            ...
            ((1.0 / 4.0) * (tmpTab[i].y + details[j][i].y))+((3.0 / 4.0) * (tmpTab[iplus1].y-details[j][iplus1].y))
            	...
            	tableauResultat.append(Point(
            ((3.0 / 4.0) * (tmpTab[i].x))+((1.0 / 4.0) * (tmpTab[iplus1].x)) ,
            ((3.0 / 4.0) * (tmpTab[i].y ))+((1.0 / 4.0) * (tmpTab[iplus1].y))
            ))
                tableauResultat.append(Point(
            ((1.0 / 4.0) * (tmpTab[i].x ))+((3.0 / 4.0) * (tmpTab[iplus1].x)) ,
            ((1.0 / 4.0) * (tmpTab[i].y ))+((3.0 / 4.0) * (tmpTab[iplus1].y))
            ))
            i=i+1
        j = j-1
...
def calculTauxErreur(tabPoint,tableauRecomposer):
    tabPX=[]
    tabPY=[]
    tabRX=[]
    tabRY=[]
    ...
    err=0
    total=0
    if len(tableauRecomposer)==0:
        return 100
    for i in range (0, len(tableauRecomposer)):
    	#i= j%len(tableauRecomposer)
    	total+=1
    	errX += math.fabs(tableauRecomposer[i].x - tabPoint[i].x)
    	errY += math.fabs(tableauRecomposer[i].y - tabPoint[i].y)
...
if __name__ == "__main__":
    ...
    while coefIgnore <5:
        ...
        tabTauxErreur.append(calculTauxErreur(tabPoint,tableauRecomposer))
        coefIgnore+=0.1
    print "tabTauxErreur",tabTauxErreur
